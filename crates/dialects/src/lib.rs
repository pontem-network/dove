...
impl Dialect for DFinanceDialect {
    fn preprocess_and_validate_account_address(&self, s: &str) -> Result<String> {
        ...
        dfinance::parse_account_address(&s).map(|address| format!("0x{}", address))
    }
    ...
    fn check_with_compiler(
        ...
        dfinance::check_with_compiler(current, deps, sender)
    }
    ...
    fn compile_and_run(
        ...
        let genesis_write_set = dfinance::resources::changes_into_writeset(genesis_changes)?;
        dfinance::executor::compile_and_run(script, deps, sender_address, genesis_write_set)
    }
    ...
    fn print_compiler_errors_and_exit(
        ...
        dfinance::report_errors(files, errors)
    }
}
