...
use crate::dfinance::resources::{ResourceStructType, ResourceWriteOp};
use crate::dfinance::vm_status_into_exec_status;
...
pub fn compile_and_run(
    ...
    let (compiled_script, compiled_modules) =
        crate::dfinance::check_and_generate_bytecode(fname, &script_text, deps, sender.into())?;

    let network_state =
        crate::dfinance::prepare_fake_network_state(compiled_modules, genesis_write_set);

    let serialized_script = crate::dfinance::serialize_script(compiled_script)?;
    let changed_resources =
        crate::dfinance::execute_script(sender, &network_state, serialized_script, vec![])?;
