...
mod dove_run_cmd_fail {
    ...
    fn fail() {
        ...
        let project_name = "demoproject_7";
        let project_folder = {
    ...
    /// project name: demoproject_9
    #[test]
    fn fail_with_argv_number_of_arguments_mismatch() {
        // Path to dove folder
        let dove_folder = {
            let mut folder = Path::new(".").canonicalize().unwrap();
            if folder.to_str().unwrap().find("dove").is_none() {
        ...
        // Project name and path
        let project_name = "demoproject_9";
        let project_folder = {
            let mut folder = dove_folder.clone();
            folder.push(project_name);
            folder
        ...
        // $ cargo run -- run rdemo.move -a 3 5 6
        // $ dove run rdemo.move
        {
            // project_folder/scripts/demo.move
            let script_path = {
            ...
            assert!(
                stdout.contains("NUMBER_OF_ARGUMENTS_MISMATCH"),
                "[ERROR] Command: {}. \r\nMessage: {}",
                command_string,
                stdout
            );
